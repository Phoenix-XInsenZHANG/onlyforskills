...
## OAuth Token Architecture Pattern

### Dual-Token Response for API Compatibility

**Problem**: OAuth generates app JWT tokens that are incompatible with Directus API calls (different payload structure).

**Solution**: Return both token types in OAuth response:

```typescript
// OAuth endpoint response
return res.json({
  data: {
    access_token: directusToken,     // Primary - Directus-compatible JWT
    app_token: accessToken,          // Secondary - app-specific JWT
    directus_token: directusToken,   // Explicit reference
    // ...other fields
  }
});
```
...
```typescript
// App JWT (for frontend application features)
{
  "userId": "123",
...
// Directus JWT (for Directus API calls)
{
  "id": "123",                    // Different field name
  "role": "role_id",
  "app_access": 1,                // Required by Directus
  "admin_access": false,          // Required by Directus
  "iat": timestamp,
  "iss": "directus"               // Required issuer
}
```
...
export async function POST(request: NextRequest) {
  // Frontend sends access_token (now Directus-compatible)
  const authHeader = request.headers.get('Authorization')
  const token = authHeader?.replace('Bearer ', '')

  // Use directly with Directus API - no conversion needed
  const response = await fetch(`${DIRECTUS_ENDPOINT}/comments`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ collection, item, comment })
  })
  // ✅ Works! No 401 errors
}
```

**Implementation**: See [CARD-OAUTH-008](../../../docs/cards/CARD-OAUTH-008-dual-token-response.md)
...
8. **Dual-Token Pattern**: OAuth should return both app and Directus-compatible tokens for API compatibility
